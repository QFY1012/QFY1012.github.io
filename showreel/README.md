...
| 04 NarraSteer | opaque agent; 74% idle, 5/8 saw drift only at the end | the trace, shown as a familiar agent transcript (tool call + result per step), maps into a narrative-space disc: each step flies to the attribute it analyses and the storyline is drawn through them; a drag on the disc flies back into the transcript as a user steer, and the agent carries on from it | N=16 · +13.2% insights · 15/16 steered by drag · 10/16 intervened mid-run |
| 05 ToA | novices get lost in linear chat; 4/6 failed | the numbered chat re-laid out as an analysis tree: each answer flies into a node, each question onto the edge into it (1–5 in chat order land on different branches); chart signals suggest next queries | N=12 · +58.3% insights/turn · +17.7% thinking time · −23% turns |
